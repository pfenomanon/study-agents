...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetchWithRetry(`${BASE}/copilot/cag-process`, {
      method: "POST",
      headers,
      body: JSON.stringify({ path, profile_id: profileId || undefined }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      return NextResponse.json({ error: data?.detail || data?.error || "cag_error" }, { status: res.status });
    }
