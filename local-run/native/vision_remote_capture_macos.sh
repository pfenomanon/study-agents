...
while true; do
  ...
  x=$((bx + left_px))
  y=$((by + top_px))
  w=$((screen_w - left_px - right_px))
  h=$((screen_h - top_px - bottom_px))

  if [[ "$w" -lt 64 || "$h" -lt 64 ]]; then
    echo "Invalid capture region after margins. width=$w height=$h" >&2
    exit 1
  fi

  tmp_png="$(mktemp /tmp/study-agents-capture.XXXXXX.png)"
  trap 'rm -f "$tmp_png"' EXIT
  screencapture -x -R "${x},${y},${w},${h}" "$tmp_png"
